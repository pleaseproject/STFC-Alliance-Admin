module.exports = {
    ...
    aliases: ["tag"],
    description:
      "Responds with a beer delivery to requesting user or targeted user.",
    category: "Util",
    minArgs: 1,
    maxArgs: 2,
    ...
    run: ({ message, args }) => {
      ...
      if (mentionTest > 0) {
        ...
        if (member == owner) {
          message.reply(
            "The user's nickname I am trying to change is either a higher role than me or a server owner!"
          );
          return;
        }
        ...
        if (member == owner) {
          message.reply(
            "You are the server owner and I can not change your nickname!"
          );
          return;
        }
