  ...
  run: async ({ message, args }) => {
    ...
    if (results.n === 0) {
      message.channel.send(
        `There was an error trying to delete \`\`${commandName}\`\` from the database. This may be because \`\`${commandName}\`\` is not in the database.`
      );
    } else {
      message.reply(
        `\`\`${commandName}\`\` has been removed from the database!`
      );
    }
  },
};
